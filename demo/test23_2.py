...
# 提供三个数字，以三个数字为键，三个数字的平方为值来创建字典：
dic = {x:x**2 for x in (2,4,6)} # 遍历元组
# dic = {x:x**2 for x in [2,4,6]} # 遍历列表
# dic = {x:x**2 for x in {2,4,6}}   # 遍历集合
...
# 判断不是 abc 的字母并输出：
a = {x for x in 'abasdadasadeqwwq' if x not in 'abc'}
print(a) # {'w', 'q', 'e', 'd', 's'}

# 元组推导式（生成器表达式）
a = (x for x in range(1,10) if x % 2 == 0)
print(a) # <generator object <genexpr> at 0x000001D64EF3B100> 返回的是生成器对象
print(tuple(a)) # (2, 4, 6, 8) 使用 tuple() 函数，可以直接将生成器对象转换成元组
