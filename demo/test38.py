...
'''
def test():
    msg_inner = 'i am from func test'

# print(msg_inner) # NameError: name 'msg_inner' is not defined

# 全局变量和局部变量
total = 0 # 这是1个全局变量
def sum(a1,a2):
    total = a1 + a2 # total 这里是局部变量
    print("函数内是局部变量：",total)
    return total
...
num1 = 1
def fun1():
    global num1 # 需要使用 global 声明
    print(num1)
    num1 = 123 # 这里就修改了全局变量的值
    print(num1)
...
def outer():
    num2 = 10
    def inner():
        nonlocal num2 # nonlocal 关键字声明 非局部变量
        num2 = 100 # 这里修改了 非局部变量
        print(num2)
    inner()
    print(num2)
...
def test():
    # 这里a2并没有定义，直接使用报错
    a2 = a2 + 1 # UnboundLocalError: cannot access local variable 'a2' where it is not associated with a value
    print(a2)
