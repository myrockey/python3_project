...
x = 10*3.25
y = 200*200
print('x 的值为： ' + repr(x) + ',  y 的值为：' + repr(y) + '...')
print('x 的值为： ' + str(x) + ',  y 的值为：' + str(y) + '...')
#  repr() 函数可以转义字符串中的特殊字符
hello = 'hello, runoob\n'
print(str(hello))
print(repr(hello))
...
print('PI:{!a}'.format(math.pi))
print('PI:{0:.3f}'.format(math.pi))
# 在 : 后传入一个整数, 可以保证该域至少有这么多的宽度。 用于美化表格时很有用
table = {'Google': 1, 'Runoob': 2, 'Taobao': 3}
for name,number in table.items():
    print('{0:10} => {1:10}'.format(number,name))
    print('{0:10} => {1:10d}'.format(name,number))
...
def pickle_write():
    ...
    # Pickle the list using the highest protocol available.
    pickle.dump(selfref_list, output, -1) # -1 是 “使用当前可用的最高协议版本” 的简写。
...
def pickle_read():
    ...
    data1 = pickle.load(pkl_file) #类似file readline()
    pprint.pprint(data1)
...
selfref_list.append(selfref_list)
含义
把变量 selfref_list 自身作为最后一个元素再塞进同一个列表，于是得到一个**自引用（self-referencing）**结构：
列表第 4 个元素就是列表本身，循环嵌套，理论上无限深度 [1, 2, 3, [1, 2, 3, [1, 2, 3, …]]]。
类型
整体仍然是内置类型 list，只是内部出现了循环引用（circular reference）。
Python 解释器对此完全合法，打印时会用 ... 表示递归以避免无限输出：
Python
复制
>>> selfref_list
[1, 2, 3, [...]]
用途/注意
常用于演示垃圾回收、序列化边界情况；
copy.deepcopy、pickle、json.dumps 等遇到它会报错或需要特殊处理；
内存层面只有一个列表对象，多出的“元素”只是再次引用同一个对象，不会额外复制。
'''
selfref_list = [1, 2, 3]
selfref_list.append(selfref_list)
def walk_one_level(iterable):
    for item in iterable:
        if item is iterable:   # 精确判断“就是自己”
            break
        yield item

for v in walk_one_level(selfref_list):
    print(v)